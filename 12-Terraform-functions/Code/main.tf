...
locals {
  ...
  backup_sensitive   = sensitive(local.backup_name)
}
...
locals {
  ...
  all_regions = concat(["ap-south-1"], tolist(local.regions))
}
...
locals {
  ...
  formatted_date    = formatdate("2006-01-02", local.current_timestamp)
}

# Assignment 12: File Content Handling
locals {
  # config_json = file("${path.module}/config.json")
  config_json = fileexists("${path.module}/config.json") ? file("${path.module}/config.json") : "{}"
  config_data = jsondecode(local.config_json)
  secret_data = jsonencode(local.config_data)
  config_sensitive   = sensitive(local.secret_data)
}
